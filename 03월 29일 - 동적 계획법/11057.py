...
def count_inc_num(n):                   #경우의 수 구하는 함수 
    dp = [[1]*10 for _ in range(n+1)]   #총 n가지의 경우의 수를 저장할 공간 확보 
    for length in range(2, n+1):        #2부터 하나씩 구하기 
        for digit in range(1, 10):      
            dp[length][digit] = dp[length][digit-1] + dp[length-1][digit]
            dp[length][digit] %= MOD   

    return sum(dp[n]) % MOD             #MOD로 나눈 나머지값 리턴 


n = int(input())
print(count_inc_num(n))                 #리턴한 값 출력
