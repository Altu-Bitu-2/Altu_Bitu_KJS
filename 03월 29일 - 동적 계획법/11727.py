...
def fill_tile(n):               #타일을 채우는 함수
    if n == 1:  # 1일 때는 바로 값 리턴 (인덱스 에러 방지)
        return 1

    dp = [0] * (n + 1)          #n번의 경우 값을 넣을 공간 만들기 

    dp[1] = 1                   #2x1의 경우 1가지만 가능 
    dp[2] = 3                   #2x2의 경우 3가지 가능 

    for i in range(3, n + 1):   #2x3의 경우부터 하나씩 계산 
        dp[i] = dp[i-2] * 2 + dp[i-1]
        dp[i] %= MOD    # 중간 과정에서 MOD연산을 통해 숫자를 줄임

    return dp[n]        #2xn일때의 값 리턴 
